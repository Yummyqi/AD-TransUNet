...
class MSA_OP(nn.Module):
    ...
    def __init__(self, dim, dim_head, window_size, qkv_bias=False, attn_drop=0., proj_drop=0., init_scale_values=1e-4):
        ...
        # 引入了一个可学习的gamma_msa参数,用于对注意力输出进行缩放,增加了模型的表达能力
...
# DMSA模块融合了可变形特征采样和多头自注意力机制,使得注意力计算可以自适应地关注输入特征的不同区域,同时通过分组策略降低了计算复杂度
class DMSA(nn.Module):
    def __init__(self, dim, dim_head, kernel_size, stride, qkv_bias=False, attn_drop=0., proj_drop=0., d_groups=4):
        super().__init__()
        # assert dim % dim_head == 0
        ...
        self.n_group_dim = self.dim // self.d_groups # 512 256 64// 4
        self.offset_range_factor = 2

        # 生成 动态偏移 和 调制信号(偏移量用于指导特征图的采样位置，调制信号则用于调整采样特征的强度)
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        # print("x shape:",x.shape) # [48, 256, 7, 7]
        q = self.q(x)
        ...
        # 空间偏移：它编码了对每个位置的卷积核进行空间变形的偏移量；
        # 调制系数：它编码了在应用空间偏移后,对每个位置的特征图进行放大或缩小的系数。
...
# DMSA_OP 模块在进行注意力计算之前,将输入特征图分割成多个窗口，引入了一个可学习的 gamma_mdmsa 参数,用于对注意力输出进行缩放
...
class Conv_OP(nn.Module):
    ...
    def __init__(self, dim, kernel_size, stride=1):
        ...
        self.conv1 = nn.Conv2d(dim, dim, kernel_size, stride, padding, groups=dim) #深度可分离卷积 组数等于输入通道数,从而实现了跨通道的卷积操作
...
class DCN2_OP(nn.Module):
    ...
    def forward(self, x):
        offset = self.conv1_offset(x) # 生成用于可形变卷积的偏移
        # 使用 DeformConv2d 进行可形变卷积操作,该操作根据生成的偏移对卷积核进行适当的形变,从而增强了模型对于物体形变的适应能力
...
class DepthwiseSeparableConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1):
        ...
        # 深度卷积部分，groups设置为in_channels实现深度卷积
        self.depthwise = nn.Conv2d(in_channels, in_channels, kernel_size, stride, padding, groups=in_channels)

        # 逐点卷积部分，1x1卷积核
        self.pointwise = nn.Conv2d(in_channels, out_channels, kernel_size=1)
        self.norm1 = get_norm('bn_2d')(in_channels)
        self.act1 = get_act('silu')()
...
class GLI(nn.Module):
    ...
    def __init__(self, in_dim, dim_head, window_size, kernel_size=5, qkv_bias=False, drop=0., attn_drop=0.,
                 ...
                 op_names=['msa', 'mdmsa', 'conv', 'dcn', 'dw'], d_group=4, gli_split=False, gli_weight=True, gli_ratio=None):
        super().__init__()
        self.op_names = op_names
        self.gli_split = gli_split # 对输入特征进行分割或复制
        self.gli_weight = gli_weight # TRUE对每个操作的输出进行加权求和 FALSE直接对每个操作的输出求平均
        self.gli_ratio = gli_ratio
        self.op_num = len(op_names)
        self.norm = get_norm(norm_layer)(in_dim)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity() # 正则化 防止过拟合 可以设置0.2-0.3 试试效果
        if self.op_num == 1:
            dims = [in_dim]
        else:
            if gli_split:
        ...
        self.dims = dims
        self.ops = nn.ModuleList()  # 两种操作的融合
        for idx, op_name in enumerate(op_names):
            ...
            elif op_name in ["dw", 'w']:
                op = DepthwiseSeparableConv(dims[idx], dims[idx], kernel_size=3, stride=1, padding=1)
            else:
        ...
        # self.gate_attention = GateAttention()

    def forward(self, x):
        # print("x shape:",x.shape) # [3, 512, 28, 28]
        # 调整x的结构
        # x = x.permute(0, 2, 1)
        # b, c, l = x.size()
        # # 假设 l 可以表示为两个相等的因数，即宽度和高度
        # h = w = int(l ** 0.5)  # 计算平方根，这里为14
        # # 重整 x 的形状为 [批次大小, 高度, 宽度, 通道数]
        # x = x.reshape(b, c, h, w)
        ...
        if self.op_num == 1:
            ...
            if self.gli_split:
                if self.gli_ratio:
                    ...
                    xs = torch.chunk(x, self.op_num, dim=1) # 将张量 x 沿着通道维度分割成多个较小的张量
            else:
                xs = [x] * self.op_num
            if self.gli_weight:
                alphas = F.softmax(self.alphas, dim=-1) # 应用 softmax 函数得到权重 alphas
                if self.gli_split:
                    if self.gli_ratio:
                    ...
                    # 将每个操作的输出 self.ops[i](xs[i]) 与对应的权重 alphas[i] 相乘,最后将所有加权操作输出沿通道维度进行拼接
                    xs = torch.cat([self.ops[i](xs[i]).unsqueeze(dim=-1) * alphas[i] for i in range(self.op_num)],
                                   dim=-1)
                    # reduce 函数对拼接后的张量在 n 维度上取平均值
        ...
        # print("x shape:",x.shape)[3, 768, 14, 14]
        # print("shortcut shape:",shortcut.shape) [3, 768, 14, 14]
        # x = GateAttention(x, shortcut, 384) # 先不加东西 看效果怎么样

        # x = x.contiguous().view(b, h * w, c)  # 将x的形状还原为原来形状
...
#dim_head=8改为16 意味着有dim//16个注意力头
    def __init__(self,  emb_dim, kernel_size=3, stride=1, dilations=[1, 2, 3], norms=['bn_2d', 'bn_2d', 'bn_2d'],
                 dim_head=16, window_size=7, qkv_bias=True, drop=0., attn_drop=0., drop_path=0.,
                 op_names=['mdmsa', 'dw'], d_group=4, c_group=-1, gli_split=True, gli_weight=True, gli_ratio=None,
                 mlp_ratio=4., ):
        super().__init__()

        self.layer = GLI(emb_dim, dim_head, window_size, 5, qkv_bias,
